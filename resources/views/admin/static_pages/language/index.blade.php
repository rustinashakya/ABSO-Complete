        ...
        <div class="content-header px-3">
            <div class="container-fluid">
                <div class="row mb-2 mx-1">
                    <div class="col-sm-6">
                        <h1 class="m-0">{{ __('Page Translation') }}</h1>
                    </div>
                    <div class="col-sm-6">
                                ...
                                @can('Language create')
                                    <div class="row mb-3">
                                        <div class="col-md-12 text-right">
                                            <a href="{{ route('admin.page.language.create', ['page_id' => $page->id, 'title'=> 'Static Page']) }}" class="btn btn-primary"> +
                                                Add</a>
                                        </div>
                                    </div>
                                @endcan
                                        ...
                                        <tbody>
                                            @forelse ($languages as $key => $language)
                                                <tr>
                                                    ...
                                                    <td class="d-flex">
                                                        @can('Language edit')
                                                            <a href="{{ route('admin.page.language.edit', ['page_id' => $page->id, 'id' => $language->id, 'title'=> 'Static Page']) }}"
                                                                class="btn btn-primary btn-sm mr-1" type="submit">
                                                                <i class="fas fa-edit" title='Edit'></i></a>
                                                        @endcan
                                                        @can('Language delete')
                                                            <form method="POST"
                                                                action="{{ route('admin.page.language.destroy', ['page_id'=> $page->id, 'id' => $language->id]) }}">
                                                                @csrf
                                                                @method('DELETE')
