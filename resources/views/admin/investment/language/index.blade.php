        ...
        <div class="content-header px-3">
            <div class="container-fluid">
                <div class="row mb-2 mx-1">
                    <div class="col-sm-6">
                        <h1 class="m-0">{{ __('Investment Translation') }}</h1>
                    </div>
                    <div class="col-sm-6">
                                ...
                                @can('Language create')
                                    <div class="row mb-3">
                                        <div class="col-md-12 text-right">
                                            <a href="{{ route('admin.investment.language.create',  $investment->id) }}" class="btn btn-primary"> +
                                                Add</a>
                                        </div>
                                    </div>
                                @endcan
                                            ...
                                            @forelse ($languages as $key => $language)
                                                <tr>
                                                    ...
                                                    <td class="d-flex">
                                                        @can('Language edit')
                                                            <a 
                                                            href="{{ route('admin.investment.language.edit', ['investment_id' => $investment->id, 'id' => $language->id]) }}"
                                                                class="btn btn-primary btn-sm mr-1" type="submit">
                                                                <i class="fas fa-edit" title='Edit'></i></a>
                                                        @endcan
                                                        @can('Language delete')
                                                            <form method="POST"
                                                                action="{{ route('admin.investment.language.destroy', ['investment_id'=> $investment->id, 'id' => $language->id]) }}"
                                                                >
                                                                @csrf
                                                                @method('DELETE')
