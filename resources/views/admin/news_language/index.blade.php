        ...
        <div class="content-header px-3">
            <div class="container-fluid">
                <div class="row mb-2 mx-1">
                    <div class="col-sm-6">
                        <h1 class="m-0">{{ __('News Translations') }}</h1>
                    </div>
                    <div class="col-sm-6">
                                ...
                                @can('News create')
                                    <div class="row mb-3">
                                        <div class="col-md-12 text-right">
                                            <a href="{{ route('admin.news.language.create', $news->id) }}" class="btn btn-primary"> +
                                                Add</a>
                                        </div>
                                    </div>
                                @endcan
                                    ...
                                    <table class="table table-bordered table-striped table-hover">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="text-uppercase">{{ __('Description') }}</th>
                                        ...
                                        <tbody>
                                            @forelse ($newsLanguages as $key => $newsLanguage)
                                                <tr>
                                                    <td>{{ $loop->iteration }}</td>
                                                    <td>{{ $newsLanguage->language->name ?? '' }}</td>
                                                    <td>
                                                        {{ ucwords($newsLanguage->title) }}
                                                    </td>
                                                    <td>{!! $newsLanguage->description !!}</td>
                                                    <td class="d-flex">
                                                        @can('Language edit')
                                                            <a href="{{ route('admin.news.language.edit', ['news_id' => $news->id, 'id' => $newsLanguage->id]) }}"
                                                                class="btn btn-primary btn-sm mr-1" type="submit">
                                                                <i class="fas fa-edit" title='Edit'></i></a>
                                                        @endcan
                                                        @can('Language delete')
                                                            <form method="POST"
                                                                action="{{ route('admin.news.language.destroy', ['news_id'=> $news->id, 'id' => $newsLanguage->id]) }}">
                                                                @csrf
                                                                @method('DELETE')
