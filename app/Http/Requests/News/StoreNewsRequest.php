...
class StoreNewsRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        return [
            ...
            'slug' => ['required', 'string', Rule::unique('news', 'slug')->withoutTrashed(), new SlugRule],
            'published_date' => ['required', 'date'],
            'type' => ['required', 'in:youtube,image'],
            'youtube_link' => $request->type == 'youtube' ? ['required', new ValidYouTubeLink] : 'nullable',
            'main_image' => $request->type == 'image' ? 'required|mimes:jpeg,png,jpg,gif,svg|max:2048|dimensions:width=1540,height=810' : 'nullable',
